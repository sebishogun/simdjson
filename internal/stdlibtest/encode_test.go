...
// Vendored from go1.26.2's encoding/json/encode_test.go, package-renamed,
// bound to this library through shim.go. Adaptations are commented inline
// where the vendored expectations could not be met verbatim.
...
func TestMarshalerError(t *testing.T) {
	...
	tests := []struct {
		...
		// MarshalerError is encoding/json's own type through an alias here,
		// so only its exported fields can be set: the sourceFunc row from
		// upstream cannot be built outside package json and is dropped --
		// it exercised stdlib's formatting, not this library.
		Name(""),
		&MarshalerError{Type: st, Err: fmt.Errorf(errText)},
