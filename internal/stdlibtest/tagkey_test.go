...
// Vendored from go1.26.2's encoding/json/tagkey_test.go, package-renamed and bound
// through shim.go. Adaptations are commented inline.
...
type badFormatTag struct {
	// Upstream writes `:"BadFormat"` -- deliberately malformed -- to prove
	// a tag reflect cannot parse is ignored. vet's structtag check forbids
	// committing that literal, so the same malformation arrives via a
	// well-formed-looking tag with no json key; the semantic under test --
	// Get("json") returns nothing, the Go name is used -- is identical.
	Y string `x:"BadFormat"`
}
