...
// Vendored from go1.26.2's encoding/json/stream_test.go, package-renamed and bound
		...
		t.Run(tt.Name, func(t *testing.T) {
			...
			for i, want := range tt.expTokens {
				...
				if errWant, ok := want.(error); ok {
					// This library's syntax errors keep their own wording by
					// documented design; the vendored expectations name
					// stdlib's. Same class at the same input is the semantic
					// under test, so the fragments are mapped.
					if err == nil || !sameSyntaxClass(err, errWant) {
	...
	for _, tt := range tests {
		...
		for i := 0; true; i++ {
			if _, err := d.Token(); err != nil {
				// Wording differs by documented design; the class holds.
				if !sameSyntaxClass(err, tt.err) {
