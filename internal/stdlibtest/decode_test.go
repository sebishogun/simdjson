...
func equalError(a, b error) bool {
	...
	// The vendored tables expect encoding/json's exact error values. This
	// library's syntax errors carry their own wording by documented design,
	// so syntax expectations compare by offset; typed errors compare by
	// their exported fields; everything else by text.
	if want, ok := b.(*SyntaxError); ok {
		if got, ok2 := a.(*json.SyntaxError); ok2 {
			return got.Offset == want.Offset
		}
		if got, ok2 := a.(*simdjson.SyntaxError); ok2 {
			return got.Offset == want.Offset
		}
		return false
	}
	if want, ok := b.(*UnmarshalTypeError); ok {
		got, ok2 := a.(*UnmarshalTypeError)
		return ok2 && got.Type == want.Type && got.Offset == want.Offset &&
			got.Struct == want.Struct && got.Field == want.Field
	}
...
func TestUnmarshal(t *testing.T) {
	for _, tt := range unmarshalTests {
		t.Run(tt.Name, func(t *testing.T) {
			in := []byte(tt.in)
			// stdlib pre-checked with its internal scanner here; the
			// equivalent black-box statement is that validity agrees with
			// whether an error is expected only loosely (several table rows
			// expect type errors on valid JSON), so only the Valid/stdlib
			// agreement is asserted.
			if Valid(in) != json.Valid(in) {
				t.Fatalf("%s: Valid disagrees with encoding/json", tt.Where)
			}
			if tt.ptr == nil {
				return
			}
...
func TestUnmarshalSyntax(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			// This library's syntax errors are its own *simdjson.SyntaxError
			// by documented design; the semantic under test is that malformed
			// input reports a syntax error rather than nil or a type error.
			_, okStd := err.(*SyntaxError)
			_, okOurs := err.(*simdjson.SyntaxError)
			if !okStd && !okOurs {
...
type unexportedFields struct {
	...
	// Upstream tags m2 `json:"abcd"` to prove a named tag on an unexported
	// field is ignored; vet's structtag check forbids that form, and the
	// decode path is identical either way -- the export check runs before
	// any tag is read. The input still carries an "abcd" key that must land
	// nowhere.
	m2 map[string]any
...
func TestUnmarshalEmbeddedUnexported(t *testing.T) {
	...
	tests := []struct {
		...
		ptr:      new(S1),
		out:      &S1{R: 2},
		err:      fmt.Errorf("json: cannot set embedded pointer to unexported struct: stdlibtest.embed1"),
	}, {
		...
		ptr:      new(S5),
		out:      &S5{R: 2},
		err:      fmt.Errorf("json: cannot set embedded pointer to unexported struct: stdlibtest.embed3"),
	}, {
...
func TestUnmarshalErrorAfterMultipleJSON(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.Name, func(t *testing.T) {
			...
			// equalError, not DeepEqual: this library's syntax errors keep
			// their own wording by documented design and compare by offset.
			if !equalError(err, tt.err) {
				t.Errorf("%s: Decode error:\n\tgot:  %v\n\twant: %v", tt.Where, err, tt.err)
			}
		})
...
func TestUnmarshalRescanLiteralMangledUnquote(t *testing.T) {
	...
	if gotT != wantT {
		t.Errorf("Marshal/Unmarshal roundtrip:\n\tgot:  %q (% x)\n\twant: %q (% x)\n\tmarshaled: %q", gotT, gotT.F1, wantT, wantT.F1, b)
	}
